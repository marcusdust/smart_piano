...
QStringList CMidiDeviceRt::getMidiPortList(midiType_t type)
{
    ...
    //cout<<"\nmidi port name list:"<<endl;
    MIDI_DEBUG("midi port name list:\n");

    for (i = 0; i < portNameList.size(); i++)
    {
        //cout<<portNameList[i].toAscii().data()<<endl;
        MIDI_DEBUG("%s\n", portNameList[i].toAscii().data());
    }
...
bool CMidiDeviceRt::openMidiPort(midiType_t type, QString portName)
{
    ...
    //cout<<"openMidiPortName: "<<portName.toAscii().data()<<endl;
    MIDI_DEBUG("openMidiPortName: %s\n", portName.toAscii().data());
    ...
    for(unsigned int i=0; i< nPorts; i++)
    {
        ...
        if (name == portName) // Test for a match
        {
            if (m_midiPorts[dev] >= 0)
            {
                ...
                //cout <<"close port: "<<i<<" "<<name.toAscii().data()<<endl;
                MIDI_DEBUG("close port %d, portname: %s\n", i, name.toAscii().data());
            }
